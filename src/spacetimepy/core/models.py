...
class StoredObject(Base):
    ...
    primitive_value: Mapped[str | None] = mapped_column(String, nullable=True)
    pickle_data: Mapped[bytes | None] = mapped_column(LargeBinary, nullable=True)
...
class StackSnapshot(Base):
    ...
    order_in_call: Mapped[int | None] = mapped_column(Integer, nullable=True)  # Position in the execution sequence
...
class FunctionCall(Base):
    ...
    call_metadata: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)  # For storing additional data like PyRAPL measurements
    ...
    session_id: Mapped[int | None] = mapped_column(Integer, ForeignKey('monitoring_sessions.id'), nullable=True)

    # Hierarchical structure - parent/child relationships
    parent_call_id: Mapped[int | None] = mapped_column(Integer, ForeignKey('function_calls.id'), nullable=True)

    # Ordering within the parent function (position among siblings)
    order_in_parent: Mapped[int | None] = mapped_column(Integer, nullable=True)

    # Chronological ordering within a session
    order_in_session: Mapped[int | None] = mapped_column(Integer, nullable=True)  # Position in the session sequence

    # First snapshot reference for efficient stack trace retrieval
    first_snapshot_id: Mapped[int | None] = mapped_column(Integer, nullable=True)
...
class CodeDefinition(Base):
    ...
    first_line_no: Mapped[int | None] = mapped_column(Integer, nullable=True)  # Line offset in the file
...
class MonitoringSession(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    name: Mapped[str | None] = mapped_column(String, nullable=True)  # Optional name for the session
    ...
    end_time: Mapped[datetime.datetime | None] = mapped_column(DateTime, nullable=True)  # Will be filled when session ends

    # Metadata about the session - renamed to avoid SQLAlchemy reserved name conflict
    session_metadata: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)  # For any additional data
