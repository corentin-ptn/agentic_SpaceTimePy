...
def run_explorer(
    ...
    api_process = None
    if mode in ["api", "both"]:
        from spacetimepy.interface.globalapi.api.api_controller import run_api

        if mode == "both":
            # Run the API server in a separate process if we're in 'both' mode
            import multiprocessing

            logger.info("lunching api")
            api_process = multiprocessing.Process(
                target=run_api, args=(db_file, api_host, api_port)
            )
            api_process.start()
    ...
    # if mode in ['ui', 'both']:
    #     from spacetimepy.interface.web.ui import run_ui
...
def main():
    ...
    parser.add_argument(
        "--ui-host", default="127.0.0.1", help="Host to run the UI server on"
    )
    parser.add_argument(
        "--ui-port", type=int, default=5000, help="Port to run the UI server on"
    )
    parser.add_argument(
        "--api-host", default="127.0.0.1", help="Host to run the API server on"
    )
    parser.add_argument(
        "--api-port", type=int, default=8080, help="Port to run the API server on"
    )
